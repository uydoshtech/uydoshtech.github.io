...
const CHROME_ICONS = {
  ...
  refresh: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M23 4v6h-6"></path><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"></path></svg>',
  ...
  // Admin "edit listing" floating action button (see .detail-admin-edit-fab
  // in listing-detail.css / listing-detail.js) — self-contained stroke
  // (unlike iconPencil() below, which relies on a caller-supplied CSS
  // override) since chrome icons are meant to be dropped in via [data-icon].
  pencil: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5Z"></path></svg>',
  ...
  // Header hamburger trigger + its dropdown's app-level nav items (privacy,
  // terms, delete account, contact) — see accountMenuHtml() in uydosh-mini-app.js.
  menu: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><path d="M4 6h16M4 12h16M4 18h16"></path></svg>',
  shield: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3l7 3v5c0 5-3.5 8-7 10-3.5-2-7-5-7-10V6l7-3Z"></path></svg>',
  ...
  // drawer-body svg { fill: none }` rule doesn't blank the dots out.
  moreHorizontal: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" aria-hidden="true"><path d="M5 12h.01M12 12h.01M19 12h.01"></path></svg>',
  ...
  alertCircle: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="9"></circle><path d="M12 8v5"></path><path d="M12 16.2h.01" stroke-width="3"></path></svg>',
  ...
  chatBubbles: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" aria-hidden="true"><path d="M5 7h10v7H9l-3.5 2.5V14H5V7Z"></path><path d="M9 7V5h10v7h-2"></path></svg>',
...
function chipButtonHtml({
  ...
  const contentHtml = iconAfterLabel ? `${labelHtml}${icon}` : `${icon}${labelHtml}`;
  return `<button type="button" class="${className}"${attrHtml} data-haptic="${haptic}"${pressedHtml}${ariaLabelHtml}>${contentHtml}</button>`;
}
